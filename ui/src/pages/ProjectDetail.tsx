...
export function ProjectDetail() {
  ...
  const archiveProject = useMutation({
    ...
    onSuccess: (updatedProject, archived) => {
      invalidateProject();
      const name = updatedProject?.name ?? project?.name ?? t("title");
      if (archived) {
        pushToast({ title: t("archived", { name }), tone: "success" });
        navigate("/dashboard");
      } else {
        pushToast({ title: t("unarchived", { name }), tone: "success" });
      }
    },
    onError: (_, archived) => {
      pushToast({
        title: archived ? t("archiveFailed") : t("unarchiveFailed"),
        tone: "error",
      });
    },
  });
  ...
  const projectBudgetSummary = useMemo(() => {
    ...
    return {
      ...
      scopeName: project?.name ?? t("title"),
      metric: "billed_cents",
      windowKind: "lifetime",
      amount: 0,
      observedAmount: 0,
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={activeTab ?? "list"} onValueChange={(value) => handleTabChange(value as ProjectTab)}>
        <PageTabBar
          items={[
            { value: "list", label: t("tabs.issues") },
            { value: "overview", label: t("tabs.overview") },
            ...(project.managedByPlugin ? [{ value: "plugin-operations", label: t("tabs.pluginOperations") }] : []),
            ...(showWorkspacesTab ? [{ value: "workspaces", label: t("tabs.workspaces") }] : []),
