...
export function NotFoundPage({ scope = "global", requestedPrefix }: NotFoundPageProps) {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("common:noResults") }]);
  }, [setBreadcrumbs, t]);
  ...
  const title = scope === "invalid_company_prefix" ? t("common:notFound.companyNotFound") : t("common:notFound.pageNotFound");
  const description =
    scope === "invalid_company_prefix"
      ? t("common:notFound.noCompanyPrefix", { prefix: normalizedPrefix ?? "unknown" })
      : t("common:notFound.routeNotExist");
    ...
    <div className="mx-auto max-w-2xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <div className="mt-4 rounded-md border border-border bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
          {t("common:notFound.requestedPath")}: <code className="font-mono">{currentPath}</code>
        </div>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          <Button asChild>
            <Link to={dashboardHref}>
              ...
              {t("common:notFound.openDashboard")}
            </Link>
          </Button>
