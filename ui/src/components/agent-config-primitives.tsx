...
const helpKeys = [
  "name", "title", "role", "reportsTo", "capabilities", "adapterType", "cwd",
  "promptTemplate", "model", "thinkingEffort", "chrome", "dangerouslySkipPermissions",
  "dangerouslyBypassSandbox", "search", "fastMode", "workspaceStrategy", "workspaceBaseRef",
  "workspaceBranchTemplate", "worktreeParentDir", "runtimeServicesJson", "maxTurnsPerRun",
  "command", "localCommand", "args", "extraArgs", "envVars", "bootstrapPrompt",
  "payloadTemplateJson", "webhookUrl", "heartbeatInterval", "intervalSec", "timeoutSec",
  "graceSec", "wakeOnDemand", "cooldownSec", "maxConcurrentRuns",
  "maxTurnContinuationEnabled", "maxTurnContinuationMaxAttempts", "maxTurnContinuationDelaySec",
  "budgetMonthlyCents",
];

export const help: Record<string, string> = new Proxy({} as Record<string, string>, {
  get(_, key: string) {
    if (key === "__proto__" || key === "toJSON") return undefined;
    const fallbacks: Record<string, string> = {
      ...
      cwd: "Deprecated legacy working directory fallback for local adapters.",
      promptTemplate: "Sent on every heartbeat. Keep this small and dynamic. Supports {{ agent.id }}, {{ agent.name }}, {{ agent.role }} and other template variables.",
      model: "Override the default model used by the adapter.",
      thinkingEffort: "Control model reasoning depth. Supported values vary by adapter/model.",
      command: "The command to execute (e.g. node, python).",
      localCommand: "Override the path to the CLI command (e.g. /usr/local/bin/claude, codex, opencode).",
      extraArgs: "Extra CLI arguments for local adapters, comma-separated.",
      envVars: "Environment variables injected into the adapter process. Use plain values or secret references.",
      bootstrapPrompt: "Only sent when Paperclip starts a fresh session.",
      ...
      maxTurnContinuationEnabled: "Automatically queue continuation runs when max-turn cap is reached.",
    ...
    return i18n.t(`agents:help.${key}`, fallbacks[key] ?? key);
  },
  has(_, key) { return typeof key === "string"; },
  ownKeys() { return helpKeys; },
});
